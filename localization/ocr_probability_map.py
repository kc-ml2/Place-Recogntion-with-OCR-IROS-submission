...
class OCRProbabilityMap:
    ...
    def localize_with_ocr(self, particle_instance, depth_sensor, box_center_batch, depth_img_batch, similarity_mat):
        ...
        # Case 1. Use similarity measure & Object group
        if self.config.DataConfig.USE_SIMILARITY:
        ...
        # Case 2. Do not use similarity measure
        else:
            for i, single_mu in enumerate(current_mu_array):
                max_similarity = np.max(similarity_mat[i])

                if max_similarity >= self.config.DataConfig.SIMILARITY_THRESHOLD:
                    similar_word_idx = np.argmax(similarity_mat[i])

                    specific_mat = self.mu_particle_matrix_on_firstview[:, similar_word_idx, :]
                    specific_mat = np.expand_dims(specific_mat, axis=1)

                    gaussian_result = self.gaussian_mixture_kernel(single_mu, specific_mat)
                    mat_for_product = np.concatenate([mat_for_product, gaussian_result], axis=1)
